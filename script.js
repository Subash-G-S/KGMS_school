...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // 10. HANDBOOK SEARCH FILTER
  const handbookSearch = document.getElementById('handbookSearch');
  if (handbookSearch) {
    handbookSearch.addEventListener('input', function (e) {
      ...
      sections.forEach((section) => {
        ...
        // Check if there are individual cards in this section
        const cards = section.querySelectorAll('.uniform-card, .number-card, .guidance-card');
        if (cards.length > 0) {
          let sectionCardsMatch = false;
          cards.forEach((card) => {
            const cardText = card.textContent.toLowerCase();
            if (cardText.includes(query)) {
              card.style.display = '';
              sectionCardsMatch = true;
              hasMatch = true;
            } else {
          ...
          // Hide or show the section based on card match
          if (sectionCardsMatch) {
            section.style.display = '';
          } else {
            section.style.display = 'none';
          }
        } else {
          // Fallback search check for texts inside section (e.g. motto)
          const textElements = section.querySelectorAll('h2, p, ul li');
